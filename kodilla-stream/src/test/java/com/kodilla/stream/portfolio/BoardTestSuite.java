...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        long sumOfPeriod = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(taskProgress -> taskProgress.getTasks().stream())
                .map(Task::getCreated)
                .map(time -> time.until(LocalDate.now(), ChronoUnit.DAYS) )
                .reduce(0L, (sum, current) -> sum = sum+current);

        long howMany = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .mapToLong(taskProgress -> taskProgress.getTasks().size())
                .sum();

        long averagePeriod = sumOfPeriod/howMany;
        //Then
        assertEquals(averagePeriod, 10);
    }
}